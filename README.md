...
| 层次 | 技术 | JDK 要求 |
|------|------|----------|
| 日志采集客户端 | Logback / Log4j2 Appender + TraceFilter | **JDK 8+** |
| 服务端 | Spring Boot 3.2 + Redis/RabbitMQ + ES 8 | JDK 17+ |
| 前端 | Vue 3 + Vite 4 + Element Plus + ECharts | Node.js 14.18+（推荐 18+） |
...
| `xxx-log-client-log4j2` | **JDK 8+** + Log4j2 2.x | 使用 Log4j2 的项目引用此模块 |
...
| 模块 | 说明 |
|------|------|
| `xxx-log-common` | 公共模型、枚举、工具类（含采样策略、幂等 ID 生成） |
| `xxx-log-client` | 日志采集客户端（Logback Appender + 链路追踪 + 采样） |
| `xxx-log-client-log4j2` | Log4j2 Appender（与 Logback 版配置项对齐） |
| `xxx-log-server` | 服务端（Redis/RabbitMQ 消费、ES 写入、查询 API、仪表盘、健康检查） |
| `xxx-log-demo` | 接入示例（`boot` 单体 / `cloud` Spring Cloud / `client-cloud` 链路透传扩展） |
| `xxx-log-web` | Vue3 日志查询与监控仪表盘前端 |
...
支持按应用、**环境（env）**、TraceId、级别、关键词、时间范围组合检索；关键词在列表与详情中**高亮**显示；TraceId 点击可填充搜索，日志内容双击查看详情。结果超过 10000 条时自动切换 **search_after 游标翻页**。
...
顶栏 **「仪表盘」** 入口，查看近 24h / 7d 日志概览与趋势：

- **统计卡片**：日志总量、ERROR / WARN 数、错误应用数
- **写入趋势**：按小时/天展示日志写入量折线图
- **ERROR 趋势**：按小时/天展示 ERROR 数量折线图
- **Top ERROR 消息**：按异常类名聚合，展示高频 ERROR 及出现次数

![监控仪表盘](docs/images/dashboard.png)
...
顶栏还展示 **ES / Redis 组件健康状态**（绿/红圆点）、**队列积压**、**消费失败计数**，超过阈值时队列数字变红并触发钉钉告警。
...
| 能力 | 说明 |
|------|------|
| **环境筛选** | 搜索栏支持按 `env`（dev/test/prod 等）过滤，选项来自近 7 天 ES 聚合 |
| **关键词高亮** | 列表「日志内容」列与详情抽屉中，命中关键词以 `<mark>` 高亮 |
| **深分页** | 常规 `page + size` 分页上限 10000 条；超出后前端切换游标模式，请求携带 `searchAfter` |
...
```json
{
  ...
  "startTime": 1717776000000,
  "endTime": 1717862400000,
  "size": 50,
  "searchAfter": [1717860000000, "abc123docId"]
}
```
...
| 能力 | 说明 |
|------|------|
| **ES 幂等写入** | 写入前为日志生成稳定 ID（`appName + env + timestamp + thread + logger + message` 哈希），重试不会产生重复文档 |
| **Redis 死信** | 消费后写 ES 失败时，原 JSON 回灌 `dead-letter-key`，并累加 `xxx-log:stats:consume-fail` 计数 |
| **RabbitMQ 死信** | 主队列绑定 DLX，消费端手动 ACK；写 ES 失败 `basicNack` 进入 `dead-letter-queue` |

相关配置（`application.yml`）：

```yaml
xxx-log:
  dead-letter-key: xxx-log:dead-letter      # Redis 模式死信 List
  rabbitmq:
    dead-letter-queue: xxx-log.dlq          # RabbitMQ 死信队列
```

> 运维可通过 `LLEN xxx-log:dead-letter` 查看 Redis 死信积压，必要时手动 `RPOP` 重放或清理。

### 运维可观测

| 能力 | 说明 |
|------|------|
| **组件健康** | `GET /api/health` 检测 ES / Redis / RabbitMQ 连通性，顶栏绿/红状态点每 30s 刷新 |
| **队列积压告警** | 积压超过 `queue-backlog-threshold` 时钉钉推送，冷却时间 `queue-backlog-cooldown-minutes` 内不重复 |
| **消费失败计数** | 顶栏展示累计消费失败条数（Redis 计数器） |

```yaml
xxx-log:
  alert:
    queue-backlog-threshold: 1000           # 积压告警阈值
    queue-backlog-cooldown-minutes: 30      # 告警冷却（分钟）
    queue-backlog-check-interval-ms: 60000  # 检查间隔
```

### 客户端采样（控制 ES 成本）

Logback / Log4j2 Appender 均支持 `sampleRate`（0–100，默认 100）。**ERROR、WARN 始终全量上报**，INFO / DEBUG 等按采样率随机丢弃。
...
<appender name="XXX_LOG" class="com.xxxlog.client.appender.XxxLogAppender">
    <appName>your-app-name</appName>
    <env>prod</env>
    <sampleRate>20</sampleRate>   <!-- INFO 约 20% 采样，ERROR/WARN 100% -->
    <!-- 其余配置同前 -->
</appender>
```
...
除 Logback 外，提供独立模块 `xxx-log-client-log4j2`（JDK 8 + Log4j2 2.x）：

```xml
<dependency>
    <groupId>com.xxxlog</groupId>
    <artifactId>xxx-log-client-log4j2</artifactId>
...
<XxxLog4j2 name="XXX_LOG"
           ...
           redisHost="127.0.0.1"
           redisPort="6379"
           queueKey="xxx-log:queue"/>
```

配置项与 Logback 版 `XxxLogAppender` 一致（队列类型、Redis/RabbitMQ 连接、采样率等）。
...
- `xxx-log-client-log4j2/1.0.0-SNAPSHOT/`（Log4j2 项目需要）
...
| POST | `/api/logs/search` | 分页 / 游标查询日志 | 是 |
| GET | `/api/logs/trace/{traceId}` | 按链路 ID 查询 | 是 |
| GET | `/api/logs/apps` | 获取应用列表 | 是 |
| GET | `/api/logs/envs` | 获取环境列表（近 7 天） | 是 |
| GET | `/api/logs/queue/stats` | 队列积压、阈值、消费失败数 | 是 |
| GET | `/api/logs/indices` | 日志索引列表及保留策略 | 是 |
| DELETE | `/api/logs/indices/{indexName}` | 删除指定日志索引 | 是 |
| GET | `/api/health` | ES / Redis / RabbitMQ 健康状态 | 是 |
| GET | `/api/dashboard/overview` | 仪表盘概览（总量、ERROR、WARN） | 是 |
| GET | `/api/dashboard/error-trend` | ERROR 趋势（按小时/天） | 是 |
| GET | `/api/dashboard/ingest-trend` | 写入量趋势（按小时/天） | 是 |
| GET | `/api/dashboard/top-errors` | Top ERROR 消息排行 | 是 |

## 核心特性

- 清晰的多模块分层，职责单一
- **采集客户端 JDK 8 编译**，兼容 Spring Boot 2.x / 3.x 业务项目
- **Logback + Log4j2 双 Appender**，配置项对齐，支持客户端采样限流
- 服务端基于 JDK 17 + Spring Boot 3，采用当前主流技术栈
- 使用 ES Java API Client 8.x，类型安全；写入幂等，消费失败进死信
- 链路追踪基于标准 MDC + HTTP Header 透传
- 客户端可选 Redis / RabbitMQ 队列，异步批量推送，不阻塞业务线程
- 支持 Spring Cloud 微服务链路透传（Feign / Gateway / B3 / W3C）
- Vue3 + Element Plus 现代化查询界面，关键词高亮、env 筛选、深分页游标
- **监控仪表盘**：ERROR / 写入量趋势、Top ERROR 排行（ECharts）
- **运维可观测**：ES/Redis 健康状态、队列积压阈值钉钉告警、消费失败计数
- 索引拆分策略可配置（时/日/月）
- **日志索引管理**：Web 控制台查看/删除索引，可配置保留天数自动清理
- **钉钉机器人**：ERROR 关键词实时告警 + 队列积压告警 + 每日 ERROR 日总结
